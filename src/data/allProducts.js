...
const casual = Array.from({ length: 12 }, (_, i) => ({
    id: `casual_${i + 1}`,
...
const sneakers = Array.from({ length: 13 }, (_, i) => ({
    id: `sneakers_${i + 1}`,
    name: `Sneakers ${i + 1}`,
    category: 'Sneakers',
...
const women = Array.from({ length: 12 }, (_, i) => ({
    id: `women_${i + 1}`,
    name: `Women ${i + 1}`,
    category: 'Women',
...
const differentProduct = [...casual, ...formal, ...slides, ...sneakers, ...women];
export const allProducts = shuffle(differentProduct);
